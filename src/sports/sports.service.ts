...
export class SportsService {
  ...
  async fetchFixtures({
    ...
    if (leagueId) {
      ...
      params.append(
        'season',
        new Date(new Date().toUTCString()).getFullYear().toString(),
      );
    }
    ...
    const data = await this.apiService.apiSportGET<any>(
  ...
  async fetchNFLGames({ page = 0, limit = 0 }: PaginationDTO) {
    ...
    const data = await this.apiService.apiSportGET<any>(
      `/games?league=1&season=${new Date(new Date().toUTCString()).getFullYear()}`,
    );
    fixtures = data.response;
  ...
  async placeFootballBet(
    ...
    const timeLeft =
      (new Date(currentTournament.end).getTime() - now.getTime()) / (1000 * 60);

    const THRESHOLD = 10;

    let formattedTime =
      timeLeft > 60
        ? `${Math.floor(timeLeft / 60)} Hrs`
        : `${timeLeft} minutes`;

    if (timeLeft <= THRESHOLD) {
      throw new UnprocessableEntityException(
        `The current tournament will end in ${formattedTime}. You can't place a bet.`,
      );
    }
    ...
    let res = await this.apiService.apiSportGET<any>(
  ...
  async placeNFLBet(
    ...
    if (game.game.status.short !== 'NS') {
      throw new BadRequestException(
        "Sorry, you can't bet on this game right now",
      );
    }
    ...
    const bet = await this.prisma.sportBet.create({
      data: {
        ...
        placebetOutcome,
        stake,
        elapsed: game.game.status.timer,
        goals: {
